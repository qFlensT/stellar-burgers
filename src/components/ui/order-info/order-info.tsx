...
export const OrderInfoUI: FC<OrderInfoUIProps> = memo(
  ({ orderInfo, printOrderNumber = false }) => (
    <div className={styles.wrap}>
      {printOrderNumber && (
        <p className={clsx('text text_type_digits-default', styles.number)}>
          #{orderInfo.number}
        </p>
      )}
      <h3 className={`text text_type_main-medium pt-5 ${styles.header}`}>
        {orderInfo.name}
      </h3>
